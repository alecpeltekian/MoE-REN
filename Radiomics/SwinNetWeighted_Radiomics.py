...
class LobeAttentionModule(nn.Module):
    """
    Improved attention module that properly weights features based on lobe importance
    """
    def __init__(self, feature_size):
        super(LobeAttentionModule, self).__init__()
    ...
    def forward(self, x, lobe_masks, lobe_weights):
        ...
        lobe_masks: One-hot encoded lobe masks [B, 5, H, W, D] 
        lobe_weights: Importance weights for each lobe [B, 5]
        """
        ...
        for i in range(5):
            ...
            lobe_mask = lobe_masks[:, i:i+1, :, :, :]


            lobe_weight = lobe_weights[:, i].view(batch_size, 1, 1, 1, 1)


            weighted_attention += attention_base * lobe_mask * lobe_weight
...
class LobeImportanceGuidedTransformer(nn.Module):
    """
    Enhanced Swin-UNETR with corrected lobe importance guidance
    """
    def __init__(self, img_size, in_channels, feature_size, lobe_importance_file, task, use_auc=False,
        ...
        self.lobe_attention = LobeAttentionModule(feature_size)
    ...
    def get_lobe_weights(self, batch_size, device):
        """Generate tensor of lobe weights with improved scaling"""
        weights = torch.ones(batch_size, 5).to(device)

        if self.lobe_importance:
            print(f"Applying lobe weights for fold {self.current_fold}:")
            for i, lobe_num in enumerate(['1', '2', '3', '4', '5']):
                if lobe_num in self.lobe_importance:
                    auc_value = self.lobe_importance[lobe_num]
                    ...
                    weights[:, i] = weight
                    print(f"  Lobe {lobe_num}: AUC={auc_value:.3f} -> Weight={weight:.3f}")


        return weights

    def forward(self, x, lobe_masks=None, radiomics_features=None):
        """Forward pass with corrected lobe importance guidance"""
        batch_size = x.size(0)
        device = x.device
        ...
        if lobe_masks is not None:
            ...
            lobe_weights = self.get_lobe_weights(batch_size, device)
...
def debug_lobe_weights(model, sample_batch, device):
    ...
    with torch.no_grad():
        ...
        for i in range(5):
            lobe_auc = model.lobe_importance.get(str(i+1), 'N/A')
            weight = lobe_weights[0, i].item()
            print(f"  Lobe {i+1}: AUC={lobe_auc}, Weight={weight:.3f}")


        lobe_masks = sample_batch["lobe_mask"].to(device)
        if lobe_masks.shape[1] == 1:

            lobe_masks_one_hot = torch.zeros(
                (lobe_masks.size(0), 6, *lobe_masks.shape[2:]),
                device=device
            )
            for i in range(6):
                lobe_masks_one_hot[:, i] = (lobe_masks[:, 0] == i)
            lobe_masks = lobe_masks_one_hot[:, 1:6]


        for i in range(5):
            mask_sum = lobe_masks[:, i].sum().item()
            mask_total = lobe_masks[:, i].numel()
            print(f"  Lobe {i+1} mask coverage: {mask_sum/mask_total*100:.1f}%")
...
def validate_weight_setup(model, train_loader, device, fold):
    ...
    debug_lobe_weights(model, sample_batch, device)
...
def train_model_with_lobe_importance(task, lobe_masks_dir, radiomics_dir=None,
                                    lobe_importance_file='lobe_importance_xgboost.json',
                                    use_auc=False, use_relative=True, resume_dir=None,
                                    single_fold=None):
    """
    Train model with lobe importance guidance and optional radiomics features
    
    Args:
        task: Task name (e.g., 'death_3')
        ...
        lobe_importance_file: Path to lobe importance JSON file
    ...
    if resume_dir:
        ...
        if single_fold is not None:
            logdir = os.path.join(BASE_LOGDIR, f"{task}_lobe_guided_fold_{single_fold}_{timestamp}")
        else:
            logdir = os.path.join(BASE_LOGDIR, f"{task}_lobe_guided_{timestamp}")
    ...
    if fold_results:
        overall_results = {
            ...
            'lobe_importance_type': 'auc' if use_auc else 'relative_importance',
...
def evaluate_model_with_lobe_masks(model, data_loader, loss_function, device, radiomics_features=None, radiomics_feature_size=100):
    """
    Evaluate model on given data loader with lobe masks
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Train Swin-UNETR model with lobe importance guidance')
    ...
    parser.add_argument('--use_auc', action='store_true',
                      help='Use AUC values instead of relative importance')
    parser.add_argument('--use_relative', action='store_true',
                      help='Use relative importance values (default)')
    ...
    parser.add_argument('--lobe_importance_file', type=str, default='1_lobe_predictive_power_results/lobe_importance_xgboost.json',
                      help='Path to lobe importance JSON file')
    parser.add_argument('--fold_auc_file', type=str, default=None,
                      help='Path to fold-specific AUC results file (overrides lobe_importance_file)')
    ...
    lobe_importance_file = args.fold_auc_file if args.fold_auc_file else args.lobe_importance_file

    print(f"\nStarting lobe-guided training for task: {args.task}")
    print(f"Using GPU: {args.gpu}")
    print(f"Batch size: {batch_size}")
    print(f"Learning rate: {lr}")
    print(f"Early stopping patience: {patience}")
    print(f"Maximum epochs: {max_epochs}")
    print(f"Using {'AUC' if args.use_auc else 'relative importance'} values")

    if args.fold is not None:
        print(f"Training single fold: {args.fold}")

    if args.fold_auc_file:
        print(f"Using fold-specific AUC values from: {args.fold_auc_file}")
    else:
        print(f"Using lobe importance from: {args.lobe_importance_file}")
