...
BASE_LOGDIR = "/data2/akp4895/deeplearningbasicresults"
...
def evaluate_model(model, data_loader, loss_function, device):
    ...
    with torch.no_grad():
        for batch in tqdm(data_loader, desc="Evaluating"):
            ...
            outputs = model.classification_head(features)
...
class AttentionExtractor:
        ...
        for name, module in self.model.named_modules():
            if "attn" in name.lower() and hasattr(module, "forward"):
                hook = module.register_forward_hook(
                    lambda module, input, output, name=name: self.hook_fn(module, input, output, name)
                )
    ...
    def hook_fn(self, module, input, output, name):
        ...
        if attn_map is not None and isinstance(attn_map, torch.Tensor):
            ...
            self.attention_maps[name].append(attn_map.detach().cpu())
    
    def reset(self):
        self.attention_maps = {}
        
    def extract_attention_maps(self, data_loader, device, batch_size=4):
        self.model.eval()
        self.reset()
        self.scan_ids = []
        
        with torch.no_grad():
            for batch in tqdm(data_loader, desc="Extracting attention maps"):
                inputs = batch["image"].to(device)
                scan_ids_batch = batch["scan_id"]
                self.scan_ids.extend(scan_ids_batch)
                
                _ = self.model(inputs)
        
        return self.process_attention_maps()
    
    def process_attention_maps(self):
        os.makedirs(self.output_dir, exist_ok=True)
        processed_maps = {}
        
        for layer_name, attention_list in self.attention_maps.items():
            layer_dir = os.path.join(self.output_dir, layer_name.replace(".", "_"))
            os.makedirs(layer_dir, exist_ok=True)
            
            if attention_list:
                all_maps = torch.cat(attention_list, dim=0)
                
                for i, scan_id in enumerate(self.scan_ids):
                    if i < all_maps.shape[0]:
                        attn_map = all_maps[i].numpy()
                        attn_file = os.path.join(layer_dir, f"{scan_id}_attn.npy")
                        np.save(attn_file, attn_map)
                        ...
                        try:
                            if len(attn_map.shape) >= 3:
                                ...
                                nifti_img = nib.Nifti1Image(attn_map, np.eye(4))
                                nifti_file = os.path.join(layer_dir, f"{scan_id}_attn.nii.gz")
                                nib.save(nifti_img, nifti_file)
                        except Exception as e:
                            print(f"Error saving NIfTI for {scan_id}: {e}")
                
                processed_maps[layer_name] = {"shape": list(all_maps.shape)}
        
        metadata = {
            "scan_ids": self.scan_ids,
            "layer_info": processed_maps
        }
        metadata_file = os.path.join(self.output_dir, "attention_metadata.json")
...
def train_model(task, resume_dir=None):
    if resume_dir:
        ...
        logdir = os.path.join(BASE_LOGDIR, f"{task}_{timestamp}")
    ...
    for fold_idx in range(starting_fold, k_folds):
        ...
        if os.path.exists(pretrained_path):
            ...
            for key, value in ssl_weights.items():
                if key.startswith("module."):
                    key = key.replace("module.", "swinViT.")
                    key = key.replace("mlp.fc1", "mlp.linear1")
                    key = key.replace("mlp.fc2", "mlp.linear2")
                    
                    if key in model.state_dict() and value.shape == model.state_dict()[key].shape:
                        monai_loadable_state_dict[key] = value
            
            model_update_dict = model.state_dict()
        ...
        attention_output_dir = os.path.join(logdir, f"fold_{fold}_attention_maps")
        os.makedirs(attention_output_dir, exist_ok=True)
        attention_extractor = AttentionExtractor(model, attention_output_dir)
        attention_metadata = attention_extractor.extract_attention_maps(test_loader, device, batch_size)
        ...
        fold_summary = {
            ...
            'attention_maps_extracted': True,
            'attention_layers_count': len(attention_metadata)
        }
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Train Swin-UNETR model for a specific task')
