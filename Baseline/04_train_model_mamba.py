...
if MAMBA_AVAILABLE:
    ...
    class Mamba3D(nn.Module):
        def __init__(self, img_size=(96, 96, 96), patch_size=(8, 8, 8), in_channels=1, 
                     d_model=128, n_layers=3, d_state=32, d_conv=4, expand=2, 
    ...
    class BaselineMamba(nn.Module):
        def __init__(self, input_size=(96, 96, 96), dropout_rate=0.2):
...
def train_model(task, resume_dir=None):
    ...
    if resume_dir:
        ...
        logdir = os.path.join(BASE_LOGDIR, f"{task}_mamba_{timestamp}")
    ...
    for fold_idx in range(starting_fold, k_folds):
        ...
        model = BaselineMamba(input_size=(96, 96, 96), dropout_rate=0.2)
        model.to(device)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Train Mamba baseline model for a specific task')
    ...
    print(f"\nStarting Mamba baseline training for task: {args.task}")
